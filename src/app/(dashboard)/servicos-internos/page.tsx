...
interface Service {
  id: string; name: string; category: string; basePrice: number; unit: string; status: string;
}

const statusBadge: Record<string, string> = { ATIVO: "bg-green-100 text-green-800", INATIVO: "bg-gray-100 text-gray-800" };
...
export default function ServicosInternosPage() {
  ...
  function fetchData() {
    setLoading(true);
    fetch(`/api/servicos?page=${page}&limit=10&type=INTERNO`)
      .then((r) => r.json()).then((result) => { setServices(result.data); setTotalPages(result.totalPages); })
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ name: form.name, description: form.description, category: form.category, defaultPrice: form.basePrice ? Number(form.basePrice) : null, unit: form.unit, serviceType: "INTERNO" }),
      });
  ...
  if (error) return <div className="flex min-h-[60vh] items-center justify-center"><p className="text-destructive font-medium">Erro ao carregar dados.</p></div>;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">Serviços Internos</h1>
          ...
          {displayed.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground text-sm">Nenhum registo.</TableCell></TableRow> :
            displayed.map((svc: any) => {
              ...
              <TableRow key={svc.id} className={isMatch && hasAnyFilter ? "bg-primary/5 border-l-2 border-l-primary" : (!isMatch && hasAnyFilter ? "opacity-40" : "")}>
                ...
                <TableCell className="text-sm"><Badge variant="secondary" className={statusBadge[svc.status] ?? ""}>{svc.status === "ATIVO" ? "Ativo" : "Inativo"}</Badge></TableCell>
