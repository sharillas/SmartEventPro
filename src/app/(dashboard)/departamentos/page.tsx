...
export default function DepartamentosPage() {
  ...
  const { matched, rest } = useMemo(() => {
    ...
    if (!hasAnyFilter) return { matched: departments, rest: [] };
    const m: typeof departments = []; const r: typeof departments = [];
    for (const d of departments) {
      let ok = true;
      if (lf.name && !(d.name || "").toLowerCase().includes(lf.name)) ok = false;
      if (ok) m.push({ ...d, _match: true } as any); else r.push(d);
    }
    return { matched: m, rest: r };
  }, [departments, filters, hasAnyFilter]);
  ...
  async function fetchData() {
    setLoading(true);
    try {
      const res = await fetch(`/api/departamentos?page=${page}&limit=10`);
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const url = editId ? `/api/departamentos/${editId}` : "/api/departamentos";
      ...
      toast.success(editId ? "Atualizado com sucesso." : "Departamento criado.");
      fetchData();
      setNewName("");
      setModalOpen(false);
    } catch {
      toast.error(editId ? "Erro ao atualizar." : "Erro ao criar departamento.");
    } finally {
      setSubmitting(false);
    }
  }

  function handleEditClick(dep: { id: string; name: string }) {
    setEditId(dep.id);
    setNewName(dep.name);
    setModalOpen(true);
  }
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/departamentos/${deleteId}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Failed");
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-center gap-2 p-3 bg-card border border-border rounded-lg">
        <div className="flex flex-col gap-0.5">
          ...
          <input placeholder="Nome" value={filters.name} onChange={e => setFilter("name", e.target.value)} className="w-[200px] h-8 text-sm bg-background border border-border rounded px-2 text-foreground placeholder:text-muted-foreground/50 outline-none focus:border-primary/50" />
        </div>
        {hasAnyFilter && (
            ...
            <div className="overflow-x-auto">
              <div className="min-w-[900px]">
                <Table>
                  ...
                  <TableBody>
                    {displayed.map((d: any) => (
                      <TableRow key={d.id} className={`border-border ${d._match && hasAnyFilter ? "bg-primary/5 border-l-2 border-l-primary" : ""} ${!d._match && hasAnyFilter ? "opacity-40" : ""}`}>
                        <TableCell className="text-foreground font-medium whitespace-nowrap text-sm">{d.name}</TableCell>
      ...
      <Dialog open={modalOpen} onOpenChange={(open) => { if (!open) { setEditId(null); setNewName(""); } setModalOpen(open); }}>
        <DialogContent className="bg-card text-card-foreground border-border">
          <DialogHeader><DialogTitle>{editId ? "Editar Departamento" : "Novo Departamento"}</DialogTitle></DialogHeader>
          <Input
