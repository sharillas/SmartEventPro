...
# ---- Pedir domínio (opcional alterar) ----
read -p "Alterar domínio? (Enter para manter ${DOMAIN}): " NEW_DOMAIN
...
# ---- SSL ----
echo ""
read -p "Configurar HTTPS com Let's Encrypt para ${DOMAIN}? (s/N): " SSL
if [ "$SSL" = "s" ] || [ "$SSL" = "S" ]; then
  certbot --nginx -d ${DOMAIN} --non-interactive --agree-tos --email admin@${DOMAIN} 2>/dev/null || certbot --nginx -d ${DOMAIN}
fi
